...
public class CoffeeMachineDTO {
	...
	public void UnmarshallingDrinks() {
		try {
			...
			JAXBContext jaxbContext = JAXBContext
					.newInstance(CoffeeMachineDTO.class);

			Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
			CoffeeMachineDTO cfdOut = (CoffeeMachineDTO) jaxbUnmarshaller
					.unmarshal(file);
	...
	public DrinksContainer getDrinksContainer() {
		...
		for (int i = 0; i < drinksDTO.size(); i++) {
			drinksContainer.add(new Drink(drinksDTO.get(i).getName(), drinksDTO
					.get(i).getPrice()), drinksDTO.get(i).getAmount());
		}
